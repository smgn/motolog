...
public class Summarize extends AsyncTask<Object, String, Bundle> {
	View confview;
	int firstOdo,lastOdo,days,entries;
	double cash, cashperelem;
	double cashperday, kmperday;
	TextView totalcashView,totalkmView,cashperkmView,cashperdayView,entriesView,cashperelementvalue,countperelementvalue,countperelementLabel;
	Spinner maintElemSpinner;
	String dateMinTxt;
	Date bikedate ,cursordateMin,cursordateMax,selectedMinDate,dateMin, dateMax;
	int daysBetween;
	...
	protected Bundle doInBackground(Object... params) {
		...
		if (action == 1) {
			...
			mType.add("...");
			b.putStringArrayList("elements", new ArrayList<>(mType));
	...
	protected void onPostExecute(Bundle b) {
		...
		if (b.getInt("action") == 1) {
			...
			fromView.setText(sdf.format(cursordateMin));
			...
			totalkmView.setText(String.valueOf(b.getInt("totalkm")) + "km");
			cashperkmView = (TextView) confview.findViewById(R.id.cashperkm);
			cashperkmView.setText(String.valueOf(b.getDouble("cashperkm")) + "$" + "/km");
